...
# ---------------------------------------------------------------------
#  • Restores the original “speed // 50” gear-and-clutch logic.
#  • We command the gearbox; TORCS still reports its actual gear & RPM,
#    which we log unchanged.  A `commandedGear` column is added for
#    debugging (so you can cross-check with the simulator gear column).
# ---------------------------------------------------------------------
...
STUCK_SECONDS     = 15.0
TARGET_SPEED_LOW  = 5.0        # m/s – let ML brake only above this speed
...
class Driver:
    ...
    def __init__(self, stage: int):
        ...
        # effectors / helpers
        self.gear   = 1                      # *commanded* gear we send
        self.clutch = 0.0
        self.steer  = self.accel = self.brake = 0.0
        self.meta   = 0
        self.focus_angle = 0
        ...
        if self.logfile.tell() == 0:               # header only once
            self.logger.writerow([
                ...
                "is_ml","left","right","up","down","r",
                "commandedGear"           # extra debug column
            ])
        print(f"[LOG] → {log_fn}")
    ...
    def drive(self, msg):
        ...
        # 2) simulator sensors  (gear & rpm = actual TORCS values)
        sim_speedX = s.getSpeedX() or 0.0
        sim_speedY = s.getSpeedY() or 0.0
        sim_speedZ = s.getSpeedZ() or 0.0
        sim_angle  = s.getAngle()  or 0.0
        ...
        # 3) ---------------- original gear-and-clutch logic -------------------
        cmd_gear = self.gear  # default: keep last command

        # a) manual reverse / first-gear helpers
        ...
        # b) automatic forward shifting based on speed (÷50 rule)
        if cmd_gear != -1:  # not in reverse
        ...
        # 4) choose steering / throttle / brake (ML or keyboard)
        if ml_active:
            self.accel = self.brake = 0.0
            ml_sensors = {
        ...
        # 5) focus rays (optional eye-ray trick)
        if FOCUS_ACTIVE:
            lap = s.getCurLapTime() or 0.0
            if lap - self.last_focus_time >= 1.0:
                self.focus_angle = 15 if self.steer>0.05 else -15 if self.steer<-0.05 else 0
        ...
        # 8) log (real gear & rpm + commandedGear)
        self.logger.writerow([
            self.track_name, self.car_model,
            *[sim_speedX, sim_speedY, sim_speedZ,
              sim_angle, sim_trackPos, sim_rpm, sim_gear,
              sim_distRaced, sim_damage],
            self.steer, self.accel, self.brake,
            self.clutch, self.meta, self.focus_angle,
            int(ml_active),
            int(left), int(right), int(up_key), int(down_key), int(rst),
            self.gear                          # commandedGear debug
        ])
        self.logfile.flush()
    ...
    # parse quickrace.xml to get track + car names (silently falls back)
    def _get_track_and_car(self):
        try:
