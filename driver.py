# driver.py  — hybrid TORCS client (ML + keyboard)
# ---------------------------------------------------------------------
...
class Driver:
    ...
    def __init__(self, stage: int):
        ...
        # ─── CSV logger ───
        # one file per track+car combo; header only when file is first created
        log_fn = self._make_log_filename()
        first_time = not os.path.isfile(log_fn)
    ...
    def drive(self, msg):
        ...
        new_gear = self.gear
        if down_key and sim_speedX < 1: new_gear = -1
        elif up_key and new_gear < 1:   new_gear = 1
        if new_gear != -1:
        ...
        if not ml_active:
            if self.gear == -1:
                self.accel = min(self.accel + ACC_INC, 1.0) if down_key else max(self.accel - ACC_DEC, 0.0)
                self.brake = min(self.brake + BRK_INC, 1.0) if up_key   else max(self.brake - BRK_DEC, 0.0)
            else:
                self.accel = min(self.accel + ACC_INC, 1.0) if up_key   else max(self.accel - ACC_DEC, 0.0)
                self.brake = (min(self.brake + BRK_INC, 1.0) if down_key and self.accel<0.3
                              else max(self.brake - BRK_DEC, 0.0))
        else:
            # ML branch (unchanged)
    ...
    def _get_track_and_car(self):
        """Reads quickrace.xml & scr_server.xml to return (track_name, car_model)."""
        try:
            ...
            robot = ss_root.find(
                f".//section[@name='Robots']/section[@name='index']/section[@name='{idx}']"
    ...
    def _make_log_filename(self):
        """Return a single CSV per track & car combo (no numbering)."""
        slug = lambda x: re.sub(r"[^A-Za-z0-9_.-]", "_", x)
        t = slug(self.track_name)
        c = slug(self.car_model)
